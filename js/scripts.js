...
function updateMobileFlipbookLayout() {
  if (window.innerWidth < 768) {
    // "本当に今見えてる高さ"を取得（ツールバー除外・PWA対応）
    var vh = window.visualViewport ? window.visualViewport.height : window.innerHeight;
    // 上部CTA, 下部ズーム/ナビ/余白（合計推奨230px程度、要微調整）
    var reserved = 230;
    // iOSの下部バーが消える・出る現象に強く対応
    var availableHeight = Math.max(200, vh - reserved);
    $('#flipbook').css('max-height', availableHeight + 'px');
    $('.viewer-wrapper').css('max-height', (vh - 16) + 'px');
    // サイドバーや他パーツが全画面になる場合の安全処理
    $('#thumbnail-sidebar').css({width: '100vw', left: 0, right: 0});
  } else {
    // PC時はデフォルト
    $('#flipbook').css('max-height', '');
    $('.viewer-wrapper').css('max-height', '');
    $('#thumbnail-sidebar').css({width: '', left: '', right: ''});
  }
}

// リサイズ・画面回転・初回ロードに対応
window.addEventListener('resize', updateMobileFlipbookLayout);
window.addEventListener('orientationchange', updateMobileFlipbookLayout);
document.addEventListener('DOMContentLoaded', updateMobileFlipbookLayout);

// turn.jsや他のresize関数呼び出し後にも
setTimeout(updateMobileFlipbookLayout, 300);
